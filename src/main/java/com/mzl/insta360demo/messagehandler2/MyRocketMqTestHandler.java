 ...
 */
@Component
@RocketMQMessageListener(topic = RocketMessageTopic.MY_ROCKETMQ_TEST_TOPIC, consumerGroup = "my-rocketmq-test-consumer-group"
        , consumeMode = ConsumeMode.CONCURRENTLY, messageModel = MessageModel.CLUSTERING)
public class MyRocketMqTestHandler implements RocketMQListener<String> {

    private static final Logger log = LoggerFactory.getLogger(MyRocketMqTestHandler.class);

    @Override
    public void onMessage(String message) {
        if (StringUtils.isEmpty(message)) {
            log.error("RocketMq测试处理消费者接受到空消息...");
            return;
        }

        log.info("RocketMq测试处理消费者接受到了消息, message={}", message);

        MyRocketmqTestEvent myRocketmqTestEvent = JSON.parseObject(message, MyRocketmqTestEvent.class);
        log.info("RocketMq测试处理消费者正在处理消息, event={}", myRocketmqTestEvent);

        // 休息5s，模拟消息处理核心逻辑
        try {
            Thread.sleep(5000);
        } catch (InterruptedException e) {
            log.error("RocketMq测试处理消费者处理失败...message={}", message);
        }

        log.info("RocketMq测试处理消费者处理完成");
    }
